...
import jp.co.next_evolution.sandbox.infrastructure.db.mapper.user.SandboxUserMapper;
...
public class UserRepositoryImpl implements UserRepository {
  ...
  private final SandboxUserMapper sandboxUserMapper;

  @Override
  public Optional<User> login(String userId, String email) {
    return Optional.ofNullable(sandboxUserMapper.login(userId, email))
                   .map(this::toDomain);
  }

  @Override
  public int searchCount(String emailAddress, Boolean approved) {
    return sandboxUserMapper.searchCount(emailAddress, approved);
  }

  @Override
  public List<User> search(String emailAddress, Boolean approved, int page, int size) {
    return sandboxUserMapper.search(emailAddress, approved, page, size)
                            .stream()
                            .map(this::toDomain)
  ...
  public Optional<User> findByUserId(String userId) {
    return Optional.ofNullable(sandboxUserMapper.findByUserId(userId))
                   .map(this::toDomain);
  }

  @Override
  public boolean existsByUserId(String userId) {
    return sandboxUserMapper.existsByUserId(userId);
  }

  @Override
  public void insertUser(User user) {
    if (sandboxUserMapper.insertUser(toEntity(user)) != 1) {
  ...
  public void updateNickName(User user) {
    if (sandboxUserMapper.updateNickName(toEntity(user)) != 1) {
  ...
  public void updateApproved(User user) {
    if (sandboxUserMapper.updateApproved(toEntity(user)) != 1) {
  ...
  public void updateBlocked(User user) {
    if (sandboxUserMapper.updateBlocked(toEntity(user)) != 1) {
  ...
  public void updateAdmin(User user) {
    if (sandboxUserMapper.updateAdmin(toEntity(user)) != 1) {
      throw new UpdateException("管理者権限設定");
    }
  }

  private User toDomain(SandboxUser record) {
    return User.builder()
               .id(record.getId())
  ...
  private SandboxUser toEntity(User user) {
    return SandboxUser.builder()
                      .id(user.getId())
